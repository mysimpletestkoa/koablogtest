...
export class CustomHttp extends Http {
    ...
    private addJwt(options?: RequestOptionsArgs): RequestOptionsArgs {
        ...
        // console.log('custom-http', currentUser);
        // let currentUser = JSON.parse(this.lStorage.retrieve('currentUser'));
        if (currentUser && currentUser.token) {
            options.headers.append('Authorization', 'Bearer ' + currentUser.token);
        }
        // console.log('addJWT',options);
        return options;
    }
    ...
    private handleError(error: any) {
        if (error.status === 401) {
            ...
            window.location.href = '/#/login'; // use hash (#) only, otherwise !work in production mode
        }
